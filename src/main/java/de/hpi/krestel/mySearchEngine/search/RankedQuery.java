...
public class RankedQuery implements SearchOperation<Integer> {
	...
	final TermMainIndexImpl index;
	final PageIndex titleIndex;
	final ArrayList<String> tokens;
	
	public RankedQuery(TermMainIndexImpl stemmedIndex, PageIndex titleIndex, String queryString) {
		this(stemmedIndex, titleIndex, new Tokenizer(queryString).tokenize(true));
	}
	
	public RankedQuery(TermMainIndexImpl stemmedIndex, PageIndex titleIndex, ArrayList<String> tokens) {
	...
	public List<Integer> execute(int topK) throws IOException, WordLengthException,
		...
		for (String token : tokens) {
			if (! terms.containsKey(token) ) {
				try {
					terms.put(token, index.getList(token) );
				} catch (KeyNotFoundException e) {
					Log.log(Level.ERROR, "Did not find " + token + " in index");
				}
			}
		}
